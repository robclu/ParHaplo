...
/// @param[in]  stride          The number of elements in data between successive rows
/// @tparam     Type            The type of data for the elements in the row
/// @return     If the two rows are quivalent
// ---------------------------------------------------------------------------------------------------------- 
template <typename Type>
inline bool operator()(const  Type* data  , 
                       size_t row_one_idx ,
                       size_t row_two_idx , 
                       size_t row_length  ,
                       size_t stride      )
{
    ...
    for (size_t i = 0; i < row_length && are_equal; ++i) 
        are_equal = (data + row_one_idx * stride + i)->value() == (data + row_two_idx * stride + i)->value()
                  ? true                                                            
                  : false;
    return are_equal;
}
...
/// @tparam     Type            The type of data for the elements in the column
...
{
    bool are_equal = true;                          // Assume equal
    for (size_t i = 0; i < col_length && are_equal; ++i) 
        are_equal = (data + col_one_idx + i * stride)->value() == (data + col_two_idx + i * stride)->value() 
                  ? true 
                  : false;
    return are_equal;
}
